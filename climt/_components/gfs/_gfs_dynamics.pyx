...
def assign_grid_arrays(
    ...
    cnp.double_t[:, :, :] tvirt,
    cnp.double_t[:, :] lnpsg,
    cnp.double_t[:, :, :, :] tracers,cimport numpy as cnp
import numpy as np
import cython
import copy
...
def get_spectral_arrays():
    ...
    return np.asarray(pyVirtTempSpec, dtype=np.cdouble),np.asarray(pyDivSpec, dtype=np.cdouble),np.asarray(pyVrtSpec, dtype=np.cdouble),np.asarray(pyTracerSpec, dtype=np.cdouble),\
           np.asarray(pyLnPsSpec, dtype=np.cdouble),np.asarray(pyTopoSpec, dtype=np.cdouble),np.asarray(pyDissSpec, dtype=np.cdouble),np.asarray(pyDiffProf, dtype=np.cdouble),np.asarray(pyDmpProf, dtype=np.cdouble)
    cnp.double_t[:, :, :] vorticity,
    cnp.double_t[:, :, :] divergence):
