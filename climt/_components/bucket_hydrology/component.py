...
class BucketHydrology(Stepper):
    ...
    def array_call(self, state, timestep):
        ...
        new_state['lwe_thickness_of_soil_moisture_content'][:]=state['lwe_thickness_of_soil_moisture_content']
        ...
        for col in range(num_cols):
            ...
            area_type=state['area_type'][col].astype(str)

            if area_type == 'land':

                north_wind_speed = state['northward_wind'][0][col]
                east_wind_speed = state['eastward_wind'][0][col]

                wind_speed = sqrt(pow(north_wind_speed, 2) +
                                  pow(east_wind_speed, 2))

                potential_evaporation = self._c * wind_speed * (state['surface_specific_humidity'][col] - state['specific_humidity'][0][col])

                precipitation_rate = state['convective_precipitation_rate'][col] + state['stratiform_precipitation_rate'][col]
                diagnostics['precipitation_rate'][col]=precipitation_rate

                soil_moisture = state['lwe_thickness_of_soil_moisture_content'][col]
                ...
                diagnostics['evaporation_rate'][col]=evaporation_rate
                ...
                surface_upward_sensible_heat_flux = self._c * wind_speed * (state['surface_temperature'][col] - state['air_temperature'][0][col])
                diagnostics['surface_upward_sensible_heat_flux'][col]=surface_upward_sensible_heat_flux
                diagnostics['surface_upward_latent_heat_flux'][col]=surface_upward_latent_heat_flux

                net_heat_flux = (
                    state['downwelling_shortwave_flux_in_air'][col, 0] +
                ...
                mass_surface_slab = state['surface_material_density'][col] * \
                    state['soil_layer_thickness'][col]
                heat_capacity_surface = mass_surface_slab * state['heat_capacity_of_soil'][col]

                new_state['surface_temperature'][col] = state['surface_temperature'][col] + \
                    (net_heat_flux/heat_capacity_surface * timestep.total_seconds())
                new_state['lwe_thickness_of_soil_moisture_content'][col] = min(state['lwe_thickness_of_soil_moisture_content'][col] + \
